...
export default function Navbar() {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (search.trim()) {
      navigate(`/${shopSlug}/products?search=${encodeURIComponent(search)}`);
      setSearch("");
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to={`/${shopSlug}`} className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-display font-bold text-xl text-gradient truncate">
              {brandName}
            </span>
          </Link>
          ...
          <form
            ...
            className="hidden md:flex items-center bg-dark-700 border border-white/10 rounded-full px-4 py-2 gap-2 w-72"
          >
            <svg
            ...
            {user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 bg-dark-700 border border-white/10 rounded-full px-3 py-1.5 hover:border-primary-500/50 transition-all"
                >
                  <div className="w-7 h-7 rounded-full bg-primary-500 flex items-center justify-center text-sm font-bold text-dark-900">
                    ...
                    >
                      ...
                      {isOwnerOrStaff && (
                        <Link
                          to={`/${shopSlug}/admin`}
                          onClick={() => setUserMenuOpen(false)}
                          className="flex items-center gap-3 px-4 py-3 text-sm text-primary-400 hover:bg-white/5 transition-colors"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
              ...
              <Link
                to={`/${shopSlug}/login`}
                className="bg-primary-500 hover:bg-primary-400 text-dark-900 font-semibold text-sm px-4 py-2 rounded-full transition-colors"
              >
                Sign In
              </Link>
            )}
        ...
        <AnimatePresence>
          {mobileOpen && (
            ...
            >
              <form
                onSubmit={handleSearch}
                className="flex items-center bg-dark-700 border border-white/10 rounded-full px-4 py-2 gap-2 mb-4"
              >
                <svg
                  className="w-4 h-4 text-gray-400"
                  fill="none"
              ...
              {navLinks.map((l) => (
                <Link
                  ...
                  className="block py-2.5 text-gray-300 hover:text-primary-400 transition-colors"
                >
                  {l.label}
                </Link>
              ))}
